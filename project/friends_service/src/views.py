...
def send_friend_request(request):
    ...
    if request.method == "POST":
        ...
        try:
            ...
            requests.post(add_notification_url, json=json_data, headers=headers)
...
def list_friend_requests(request):
    ...
    if request.method == "GET":
        ...
        friend_requests_list = friend_requests.objects.filter(
            receiver_username=username, status="pending"
        ).values()
        ...
        for friend_request in friend_requests_list:
            responseDTO.append(
                {
                    "id": friend_request.get("id"),
                    "sender_username": friend_request.get("sender_username")
                }
            )

        return ResponseService.create_success_response({"friends_requests" : responseDTO}, 200)
